...
- `session save` and `session restore` commands for best-effort workspace snapshotting.
...
### Fixed
- Daemon now switches both monitors on focus events (focusedmon/workspace/activewindowv2) across socket2 and native IPC to keep paired workspaces in sync.
